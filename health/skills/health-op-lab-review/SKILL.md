...
For each biomarker outside its reference range, the op calls `health-flag-out-of-range-value` to write a structured flag to `open-loops.md`. The flag records only metadata — biomarker name, severity tier (borderline, elevated, critical), collection date, panel type, and recommended action — never the raw numerical value. This design keeps PHI confined to the lab summary document and allows open-loops.md to be referenced freely.
...
The op concludes by calling `health-update-open-loops` to consolidate all new flags and auto-resolve any prior lab flags for biomarkers that have returned to normal range in this panel.

## Calls

- **Flows:** `health-build-lab-summary`
- **Tasks:** `health-flag-out-of-range-value`, `health-update-open-loops`
