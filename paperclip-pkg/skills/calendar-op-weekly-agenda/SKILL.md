...
The op begins by calling `flow-collect-deadlines` to extract all items with due dates in the next 30 days from every installed plugin vault's open-loops.md. The deadline scan also picks up any hard-deadline records stored in vault/calendar/00_current/ from prior deadline-planning sessions. Each item is tagged as urgent (≤7 days), upcoming (8-30 days), or horizon (31-60 days). The weekly agenda owns the **8-30 day window** as part of the post-merge consolidation (urgent ≤7-day items now anchor `op-daily-brief`; this op covers everything 8-30 days that needs to be scheduled this week or the next).
...
Finally, it calls `task-update-open-loops` to ensure any newly surfaced items — deadlines discovered this run that weren't previously in the calendar vault — are added to vault/calendar/open-loops.md before the week starts.
...
9. Call `task-update-open-loops` to add any newly discovered items to open-loops.md
