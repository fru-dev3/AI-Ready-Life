...
**New flags written:** When called by any calendar op, this task appends flags for: (1) deadline clusters — multiple items due within the same 3-day window that together require more focused hours than the calendar provides in that window; (2) focus time deficits — weeks where qualifying focus time falls below 6 hours (🟡) or below 4 hours (🔴); (3) unscheduled high-priority items — items from the current week's agenda that have no focus block slot assigned despite requiring deep work; (4) overdue preparation starts — items from the deadline registry where the recommended preparation start date has passed without evidence of action; (5) **approaching deadlines (≤7 days, no prep)** — when a caller passes `flag_type=approaching-deadline` with item + domain + due-date + effort, the task checks the source domain vault for preparation evidence (milestones, payments, filed documents, in-progress annotations); if none found, writes a 🔴 critical flag with prep steps and recommended start date. Existing approaching-deadline flags are escalated (with timestamped escalation history) rather than duplicated.
