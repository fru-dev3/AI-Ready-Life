...
Called by `op-market-scan` to execute the actual job board search. This flow handles the data collection layer — reading criteria, querying sources, filtering and ranking results, and returning structured data — while the op handles routing, brief writing, and pipeline logging.
