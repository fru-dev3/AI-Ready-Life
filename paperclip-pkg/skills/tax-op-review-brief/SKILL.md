...
**Entity Compliance.** *(advanced/optional — only for users with LLCs / S-corps configured.)* For each active entity: name, compliance status (CURRENT / FLAG), and any open items. One line per entity. Example: "LLC-1: Current. S-Corp: Q3 941 return due October 31 — file via payroll provider." Skipped for W-2-only filers.
...
**Document Status (during filing season, Jan–April).** Document completeness: N of M expected documents received. Missing document count by type. Example: "3 documents pending: 1099-B from [brokerage], K-1 from [partnership], 1098 from [lender]."
...
- **Ops:** `op-deadline-watch` (for next deadline data)
