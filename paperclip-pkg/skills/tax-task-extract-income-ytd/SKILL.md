...
# task-extract-income-ytd

**Cadence:** Called by `flow-build-estimate` and other flows needing YTD income figures
...
**Return structure.** The task returns a structured record (not a vault-written document) with all the above fields populated and labeled, so the calling flow (`flow-build-estimate`) can immediately use the data without additional parsing.
