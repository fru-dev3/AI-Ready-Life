...
After building the scorecard, the op calls `task-flag-stalled-goal` for any goals in vault/vision/00_current/ that have had no activity for more than 42 days. Stalled goals appear in the scorecard's "Needs Decision" section rather than being silently dropped or passively tracked.
...
3. Call `flow-build-scorecard` to compute per-domain scores from open-loops, OKR pace, milestones
4. Call `flow-score-domain-progress` to evaluate current-quarter OKR key results
...
8. Check vault/vision/00_current/ for goals with no activity in 42+ days; call `task-flag-stalled-goal` for each
9. Write scorecard to vault/vision/00_current/YYYY-MM-scorecard.md
10. Call `task-update-open-loops` to write any 🔴 domain or KR flags to vault/vision/open-loops.md
