...
**Completeness check.** After logging the new document, the op calls `flow-document-completeness` to update the full expected vs. received picture. This ensures the completeness report is always current after each intake event rather than only during scheduled reviews.
...
- **Flows:** `flow-document-completeness`
