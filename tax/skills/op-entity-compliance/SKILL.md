...
description: >
  ADVANCED / OPTIONAL — only runs for users with active LLCs / S-corps / partnerships
  configured in vault/tax/config.md. Skip entirely for W-2-only filers. Quarterly
  compliance check covering: state annual report filing deadlines and fees, franchise
  tax payment obligations (e.g., California $800 minimum, Texas margin tax), registered
  agent renewal status, S-corp reasonable salary and payroll tax deposit requirements
  (Form 941, EFTPS), K-1 issuance deadlines, and state-specific LLC requirements.
  Flags all gaps and approaching deadlines within 90 days. Triggers: "entity compliance
  check", "LLC filing due", "business entity compliance".
---

# op-entity-compliance
...
**Advanced / optional.** Only runs if `vault/tax/config.md` has one or more active entities (LLC, S-corp, partnership, C-corp). For W-2-only filers, this op is skipped entirely. If no entities are configured, exit immediately with: "No business entities found in config — entity compliance check skipped. To enable, add an `entities:` block to vault/tax/config.md."
...
- **Flows:** `flow-document-completeness` (entity scope)
