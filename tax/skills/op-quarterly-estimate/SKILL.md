...
The op calls `flow-build-estimate` to produce the full calculation. The calculation logic applies the key IRS rules:
...
**Actual method.** Projects full-year tax from YTD income, annualizes it, divides by 4. More complex but can produce a lower payment when income is declining or front-loaded. The op uses YTD income data from `task-extract-income-ytd` and applies the current year's tax brackets and standard or itemized deductions.
...
- **Flows:** `flow-build-estimate`
- **Tasks:** `task-update-open-loops` (handles deadline-alert flags), `task-update-open-loops`
